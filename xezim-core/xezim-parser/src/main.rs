...
//!   --dump-json     Print parsed AST as JSON (one file per object, fields:
//!                     file, source_text, ast, errors, warnings)
...
fn main() {
    ...
    for file in &files {
        ...
        if dump_json {
            ...
            // JSONL: one object per file, single line, ready for line-buffered consumers.
            println!("{}", serde_json::to_string(&envelope).expect("ast serialization"));
        }

        #[cfg(not(feature = "json-cli"))]
        if dump_json {
            eprintln!("--dump-json requires the json-cli feature");
            process::exit(2);
        }
    }
